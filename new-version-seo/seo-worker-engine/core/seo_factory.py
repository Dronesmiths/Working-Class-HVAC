...
TEMPLATE_PATH = os.path.join(ROOT_DIR, '_engine', 'SEO-Factory', 'seo-engine', 'TEMPLATES', 'page-template.html')
...
LOCATION_TEMPLATE_PATH = os.path.join(ROOT_DIR, '_engine', 'SEO-Factory', 'seo-engine', 'TEMPLATES', 'location-template.html')
...
SEO_GROWTH_TABS = {
    ...
    '📍 LOCATION PAGES': [['URL', 'Last Modified', 'Status', 'Location Name', 'SEO Page Title']],
    ...
    '🚀 EXPANSION ENGINE': [['Cluster', 'Suggested Page', 'Target Keyword', 'Impressions', 'Opportunity Type']],
    ...
    '🛡️ AUTHORITY RADAR': [['Cluster', 'Total Pages', 'Internal Links', 'External Backlinks', 'Total Impressions', 'Average Position', 'Gravity Score', 'Opportunity Score', 'Priority']],
    '⚔️ CLUSTER MAP': [['Cluster', 'URL', 'Pages', 'Gravity Score', 'Opportunity Score', 'Priority', 'Recommended Action']]
}
...
def cmd_sync():
    ...
    for u in all_urls:
        ...
        # Extraction of human-readable name from URL slug with hierarchy
        path_parts = loc.rstrip('/').split('/')
        if len(path_parts) > 4:
            parent_name = path_parts[-2].replace('-', ' ').title()
            child_name = path_parts[-1].replace('-', ' ').title()
            if parent_name.lower() != 'services':
                readable_name = f"{parent_name} > {child_name}"
            else:
                readable_name = child_name
        elif len(path_parts) == 4:
            readable_name = path_parts[-1].replace('-', ' ').title()
        else:
            readable_name = "Main Page"
    ...
    urls_by_category['🚀 EXPANSION ENGINE'] = expansion_suggestions
    ...
    urls_by_category['🛡️ AUTHORITY RADAR'] = radar_rows
    ...
    urls_by_category['⚔️ CLUSTER MAP'] = cluster_map_data
    ...
    # Batch Value Update to avoid 429 Quota Errors
    batch_data = []
    print(f"Preparing batch update for {len(urls_by_category)} tabs...")

    for category, values in urls_by_category.items():
        if not values and category not in MARKET_INTEL: continue
        
        if category in SEO_GROWTH_TABS:
            headers = SEO_GROWTH_TABS[category]
        else:
            mapped_category = 'Page Health'
            if category == '📂 SERVICE PAGES': mapped_category = '📂 SERVICE PAGES'
            elif category == '📍 LOCATION PAGES': mapped_category = '📍 LOCATION PAGES'
            elif category == '✍️ BLOG ARTICLES': mapped_category = '✍️ BLOG ARTICLES'
            headers = SEO_GROWTH_TABS.get(mapped_category, [['URL', 'Status', 'Last Updated']])
    ...
    # 1. Clear all tabs first (Batch Clear)
    clear_body = {'ranges': [f"'{category}'!A:Z" for category in urls_by_category.keys() if category in sheet_meta]}
    try:
        service.spreadsheets().values().batchClear(spreadsheetId=SPREADSHEET_ID, body=clear_body).execute()
    except Exception as e:
        print(f"Warning: Batch clear failed, attempting individual clears: {e}")

    # 2. Batch Update all values
    update_body = {
    ...
    try:
        ...
        # Final fallback to individual updates with long sleep
        for item in batch_data:
            service.spreadsheets().values().update(
                spreadsheetId=SPREADSHEET_ID, range=item['range'],
                valueInputOption='RAW', body={'values': item['values']}).execute()
            time.sleep(2)

    # 3. Apply Formatting (Keep individual calls with sleep as batchFormatting is complex)
    for category in urls_by_category.keys():
        if category not in sheet_meta: continue
        values = urls_by_category[category]
        if not values and category not in MARKET_INTEL: continue
        
        p_colors = [row[4] for row in values] if category == 'Cluster_Map' else None
        headers_len = len(SEO_GROWTH_TABS[category][0]) if category in SEO_GROWTH_TABS else 5
        
        apply_formatting(service, SPREADSHEET_ID, sheet_meta[category], headers_len, priority_colors=p_colors)
...
def cmd_expand_locations():
    """Generates 3 new location pages based on the California City Gold Standard template."""
    print("🚀 Running Working Class HVAC Pro-Grade Location Expansion Engine...")
    
    # 1. Load Template
    if not os.path.exists(LOCATION_TEMPLATE_PATH):
        print(f"Error: Location Template not found at {LOCATION_TEMPLATE_PATH}")
        return

    with open(LOCATION_TEMPLATE_PATH, 'r') as f:
        master_template = f.read()

    # 2. Strategy Seeding - Phase 5 Expansion (Regional Dominance)
    potential_locations = [
        {"name": "Desert View Highlands", "neighborhoods": ["Quartz Hill West", "Highland North", "AV Ranches"]},
        {"name": "White Fence Farms", "neighborhoods": ["West Lancaster", "Avenue M", "Rural Estates"]},
        {"name": "Agua Dulce", "neighborhoods": ["Sierra Pelona", "Escondido Canyon", "Sleepy Valley"]},
        {"name": "Pinon Hills", "neighborhoods": ["Phelan Border", "Highway 138", "Desert Front"]},
        {"name": "Willow Springs", "neighborhoods": ["Rosamond West", "Raceway", "Oak Creek"]}
    ]
    ...
    for loc_data in potential_locations:
        ...
        print(f"Generating Pro-Grade location page for: {loc_name}")
        os.makedirs(target_dir, exist_ok=True)
        
        # Build Neighborhood Cards
        service_areas_html = ""
        for n in loc_data["neighborhoods"]:
            service_areas_html += f"""
                <div class="feature-card" style="background: #fff; padding: 25px;">
                    <h3 style="margin-bottom: 10px; color: #1a73e8;">{n}</h3>
                    <p>Reliable HVAC service and emergency repairs for the {n} community.</p>
                </div>"""
        
        # Build FAQ Lists
        faq_items = [
            {
                "q": f"Do you provide emergency service in {loc_name}?",
                "a": f"Yes, we offer responsive 24/7 emergency HVAC repairs throughout {loc_name} to ensure your family stays safe and comfortable."
            },
            {
                "q": f"Are you familiar with {loc_name} local building codes?",
                "a": f"Absolutely. As a local Antelope Valley contractor, we ensure all installations and repairs in {loc_name} meet strict safety and efficiency standards."
            },
            {
                "q": f"Does Working Class HVAC service mobile homes in {loc_name}?",
                "a": f"Yes! We are specialists in mobile home heating and air conditioning, providing specialized care for manufactured homes in the {loc_name} area."
            }
        ]
        ...
        faq_schema_json = json.dumps(faq_schema_list, indent=8)[1:-1] # Strip brackets for embedding
        
        # Hydrate Template
        content = master_template.replace('{{TITLE}}', f"HVAC Services in {loc_name}, CA | Working Class HVAC") \
                                 .replace('{{META_DESCRIPTION}}', f"Professional AC repair and heating services in {loc_name}, CA. Serving {', '.join(loc_data['neighborhoods'])}. Call (661) 494-8075 for an honest estimate.") \
                                 .replace('{{CANONICAL_URL}}', f"{DOMAIN}/locations/{slug}/") \
                                 .replace('{{CITY}}', loc_name) \
                                 .replace('{{CITY_ENCODED}}', loc_name.replace(' ', '_')) \
                                 .replace('{{FAQ_SCHEMA}}', faq_schema_json) \
                                 .replace('{{H1}}', f"HVAC Services in {loc_name}, CA") \
                                 .replace('{{HERO_INTRO}}', f"Honest and affordable cooling solutions for the residents of {loc_name}.") \
                                 .replace('{{SERVICE_AREAS}}', service_areas_html) \
                                 .replace('{{FAQ_LIST}}', faq_list_html)

        with open(os.path.join(target_dir, 'index.html'), 'w') as f:
            f.write(content)
        
        # Update sitemap
        update_sitemap(f"{DOMAIN}/locations/{slug}/")
        generated.append(loc_name)
        count += 1
        
    print(f"✅ Successfully expanded {len(generated)} locations with Gold Standard Styling: {', '.join(generated)}")
    cmd_sync() # Report to sheet

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SEO Factory: Audit, Sync, and Growth Engine")
    
    # Simple Positional Command
    parser.add_argument('command', choices=['audit', 'sync', 'generate', 'discover', 'internal', 'update-internal', 'reinforce', 'autopilot', 'hubs', 'produce-subpages', 'sitemap', 'expand-locations'], help='Command to run')
    ...
    if args.command == 'audit':
        ...
        cmd_expand_locations()
    elif args.command == 'autopilot':
        print("Starting SEO Factory Autopilot...")
        cmd_sync()
        cmd_discover()
        cmd_expand_locations() # Added to autopilot for HVAC
        cmd_reinforce()
        cmd_internal()
